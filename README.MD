...
The other thing we are leveraging is how in chialisp wallet addresses are just encoded puzzle hashes and puzzle hashes are just compiled programming logic. Therefore when we ask another party to send value to an address we 100% control the programming logic of what happens next when that coin eventually gets spent.
